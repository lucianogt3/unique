...
def prontuario_to_dict(p: Prontuario) -> dict:
    ...
    erros_list = [erro.to_dict() for erro in p.erros]
    
    print(f"🔍 DEBUG prontuario_to_dict - ID: {p.id}")
    print(f"   📝 Responsáveis: {responsaveis_nomes}")
    print(f"   📝 Total de responsáveis: {len(responsaveis_nomes)}")
    print(f"   📝 Erros: {len(erros_list)}")
...
def calcular_estatisticas_bd(prontuarios_obj):
    ...
    tipos_erro_db = {}
    try:
        tipos_erro_query = TipoErro.query.all()
        tipos_erro_db = {t.nome: t for t in tipos_erro_query}
    except Exception as e:
        print(f"Aviso: Não foi possível carregar Tipos de Erro para estatísticas. {e}")
    ...
    for p in prontuarios_obj:
        ...
        for erro in p.erros:
            ...
            if tipo_erro_nome in tipos_erro_db:
...
def index():
    ...
    periodo_filter = request.args.get('periodo', 'todos')  # PADRÃO: TODOS OS DADOS
    data_inicio_filter = request.args.get('data_inicio', '')
    data_fim_filter = request.args.get('data_fim', '')
    
    print(f"🚀 INICIANDO DASHBOARD - PERÍODO: {periodo_filter}")

    # Carregar TODOS os dados com relacionamentos
    ...
    if periodo_filter == 'hoje':
        ...
        query = query.filter(Prontuario.data_criacao >= hoje)
    elif periodo_filter == 'semana':
        semana = datetime.now() - timedelta(days=7)
        query = query.filter(Prontuario.data_criacao >= semana)
    elif periodo_filter == 'mes':
        mes = datetime.now().replace(day=1, hour=0, minute=0, second=0, microsecond=0)
        query = query.filter(Prontuario.data_criacao >= mes)
    # 'todos' não aplica filtro

    # Ordenar por data
    query = query.order_by(Prontuario.data_criacao.desc())
    
    prontuarios_obj = query.all()
    
    print(f"📊 DADOS CARREGADOS: {len(prontuarios_obj)} prontuários")
    
    prontuarios_lista = [prontuario_to_dict(p) for p in prontuarios_obj]
    
    # 🔥 CORREÇÃO: ESTATÍSTICAS COMPLETAS
    total = len(prontuarios_lista)
    
    if total == 0:
        print("⚠️  AVISO: Nenhum dado para exibir")
        # 🔥 DADOS ZERADOS COM TODAS AS PROPRIEDADES NECESSÁRIAS
        stats = {
            ...
            "total_registrados_mes": 0, 
            "tempo_medio_auditoria": 0, 
            "taxa_conclusao": 0, 
            "percentual_com_erros": 0,
            # 🔥 PROPRIEDADES ADICIONAIS QUE O TEMPLATE PRECISA
        ...
        com_erro = sum(1 for p in prontuarios_lista if p.get('erros') and len(p.get('erros', [])) > 0)
        taxa_erros = round(100 * com_erro / total, 1) if total else 0

        # 🔥 CALCULAR ESTATÍSTICAS DETALHADAS DE ERROS
    ...
    print(f"✅ DASHBOARD PRONTO: {total} prontuários, {periodo_info}")
    ...
    return render_template('index.html', 
        ...
        stats_gerais_erros=stats_gerais_erros,  # 🔥 ADICIONAR ESTA LINHA
...
def relatorios():
    ...
    query = Prontuario.query.order_by(Prontuario.data_criacao.desc())
...
def adicionar_prontuario():
    try:
        ...
        # Processamento de erros com responsáveis específicos
        erros_processados = []
        responsaveis_envolvidos = set()
        
        if 'erros' in dados:
            if isinstance(dados['erros'], dict):
                for index, erro_data in dados['erros'].items():
                    if (isinstance(erro_data, dict) and erro_data.get('tipo') and 
                        erro_data.get('causa') and erro_data.get('responsavel_id')):
                        
                        erros_processados.append(erro_data)
                        responsaveis_envolvidos.add(erro_data['responsavel_id'])
                        
            elif isinstance(dados['erros'], list):
                for erro in dados['erros']:
                    if (isinstance(erro, dict) and erro.get('tipo') and 
                        erro.get('causa') and erro.get('responsavel_id')):
                        
                        erros_processados.append(erro)
                        responsaveis_envolvidos.add(erro['responsavel_id'])
        
        print(f"🔍 ERROS PROCESSADOS: {len(erros_processados)}")
        print(f"👥 RESPONSÁVEIS ENVOLVIDOS: {responsaveis_envolvidos}")
        
        # Validação: Se houver erros, deve haver responsáveis válidos
        if len(erros_processados) > 0 and len(responsaveis_envolvidos) == 0:
            return jsonify({
                'sucesso': False, 
                'erro': 'Cada erro deve estar vinculado a um responsável específico.'
            }), 400
        ...
        # Associar responsáveis baseado nos erros
        responsaveis_encontrados = []
        if responsaveis_envolvidos:
            responsaveis_encontrados = Responsavel.query.filter(
                Responsavel.id.in_(responsaveis_envolvidos)
            ).all()
        ...
        # Adição de erros com responsáveis específicos
        ...
        print(f"📝 ERROS ASSOCIADOS: {len(erros_processados)}")
        
        return jsonify({
            'sucesso': True, 
            'prontuario': prontuario_to_dict(novo_prontuario),
            'mensagem': f'Prontuário salvo com {len(responsaveis_encontrados)} responsável(eis) e {len(erros_processados)} erro(s)'
        })
...
def alimentacao():
    try:
        ...
        # 🔥 CORREÇÃO: Carregar responsáveis com ID e nome
        ...
        for resp in responsaveis:
            print(f"   - ID: {resp['id']}, Nome: {resp['nome']}")
        ...
        return render_template('alimentacao.html',
                                convenios=convenios,
                                setores=setores,
                                responsaveis=responsaveis,  # 🔥 Agora com ID e nome
                                tipos_erro=tipos_erro_dict,
                                status_opcoes=STATUS_OPCOES
                                )
        ...
        return render_template('alimentacao.html',
                                convenios=[],
                                setores=[],
                                responsaveis=[],  # Lista vazia mas estruturada
                                tipos_erro={},
                                status_opcoes=STATUS_OPCOES
                                )
